            ...
            if provider == "claude":
                ...
                try:
                    ...
                    if not ok:
                        ...
                        # Подпись селектора кладём в текст ошибки: по ней
                        # видно, что именно показывал браузер — пустая строка
                        # означает, что claude.ai сменил разметку пикера.
                        seen = getattr(sess, "last_model_state", "") or "(пусто)"
                        raise RuntimeError(
                            f"Не удалось переключить Claude на {model}"
                            + (f" / effort={effort}" if effort else "")
                            + ". Проверь что в браузере открыт claude.ai/new (не login и не challenge), "
                            "и повтори запуск. "
                            f"Селектор моделей показывал: {seen}"
                        )
                except RuntimeError:
                    raise
                except Exception as e:
