...
# Общая браузерная обвязка (запуск, разлочка профиля, докачка Chromium) —
# одна на всех провайдеров, см. browser_common.py.
...
class ClaudeAutomation:
    ...
    async def start(self, status_cb=None):
        ...
        _ensure_profile_not_locked(CHROME_PROFILE, site="claude.ai")

        self._context = await launch_persistent_context(self._pw, CHROME_PROFILE, say)
