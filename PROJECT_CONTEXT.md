...
- **Переключение модели/effort устойчиво к редизайну пикера.** Кнопку селектора
  ищем по списку кандидатов (`MODEL_BUTTON_SELECTORS`), состояние читаем из
  aria-label ИЛИ текста кнопки ИЛИ `aria-checked` пунктов открытого меню, а
  парсер (`_parse_model_label`) ищет имя модели и слово effort в любом месте
  строки, а не по позициям. Если состояние прочитать нечем, шаг не падает —
  доверяем тому, что клики по пунктам прошли, и пишем в лог слепок DOM
...
- **Картинки** (Banana) — `POST /image/banana/generate` асинхронно (апдейт API): POST → `task_id` → опрос `GET /video/status/{task_id}` (общий эндпоинт с видео), completed-ответ содержит `images[]` с `fifeUrl` (Google Cloud Storage signed, ~30 мин) и `mediaGenerationId`. 1–8 картинок за вызов, типичная генерация 1–3 мин, опрос каждые 10с
  ...
  - **Что НЕ надо ретраить, вынесено в типы.** `VeoPermanentError` — базовый;
    `VeoAuthError` (нет ключа, ключ не принят, кончились кредиты — 401/402/403)
    и `BananaNoImages` — наследники. Оба места, решающие «повторять или нет»
    (`_is_permanent_error`, `_is_permanent` в шаге), смотрят на тип. Пустой
    ключ раньше падал обычным `RuntimeError` и уходил в бесконечный ретрай —
    ровно та картина «0/10 готово, попытка 10, ждём 60с» без причины
  - Транзиентные ошибки тоже больше не вечные: `BANANA_MAX_ATTEMPTS` = 12
    попыток (≈11 минут с backoff), после чего промпт признаётся неудачным.
    Текст последней ошибки идёт прямо в строку прогресса шага — раньше он был
    виден только в логе
